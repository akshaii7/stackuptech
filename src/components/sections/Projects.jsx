  ...
  const projects = [
    {
      ...
      image: getAssetPath('/ecommerce-mockup.png'),
      isCustomLayout: true,
      theme: 'gold'
    },
    {
      ...
      image: getAssetPath('/lms-mockup.png'),
      isCustomLayout: true,
      theme: 'blue'
    },
            ...
            if (project.isCustomLayout) {
              return (
                <div
                  key={index}
                  ref={addToRefs}
                  className={`flex flex-col lg:flex-row gap-8 sm:gap-12 items-center group relative w-full rounded-[1.5rem] sm:rounded-[3rem] overflow-hidden p-5 sm:p-10 md:p-12 transition-all duration-700 shadow-2xl border glass ${project.theme === 'gold' ? 'border-[#D4AF37]/20 shadow-[#D4AF37]/5' : 'border-blue-500/20 shadow-blue-500/5'
                    }`}
                  style={{
                ...
                >
                  <div className="absolute inset-0 z-0 pointer-events-none opacity-50">
                    <div className={`absolute top-0 left-0 w-full h-full bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] via-transparent to-transparent ${project.theme === 'gold' ? 'from-[#D4AF37]/20' : 'from-blue-600/30'
                      }`}></div>
                    <div className={`absolute bottom-0 right-0 w-full h-full bg-[radial-gradient(ellipse_at_bottom_right,_var(--tw-gradient-stops))] via-transparent to-transparent ${project.theme === 'gold' ? 'from-[#FFD700]/10' : 'from-purple-600/30'
                      }`}></div>
                  </div>

                  {/* Project Details */}
                  <div className="w-full lg:w-1/2 flex flex-col justify-center relative z-10">
                    <h3 className={`text-3xl xs:text-4xl sm:text-5xl md:text-6xl font-bold mb-2 tracking-tight ${project.theme === 'gold' ? 'gold-text-gradient' : 'text-white'
                      }`}>{project.title}</h3>
                    <h4 className={`text-lg sm:text-2xl font-medium mb-6 sm:mb-8 tracking-wide ${project.theme === 'gold' ? 'text-[#FFD700]' : 'text-blue-400'
                    ...
                    <ul className="grid grid-cols-1 xs:grid-cols-2 gap-y-3 gap-x-4 mb-8 sm:mb-10 text-white/80 font-light text-sm sm:text-lg">
                      {project.features.map((feature, i) => (
                        <li key={i} className="flex items-center gap-2 sm:gap-3">
                          <div className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full flex-shrink-0 ${project.theme === 'gold'
                    ...
                    <div className="flex flex-col xs:flex-row gap-4 sm:gap-6">
                      <a
                        ...
                        className={`flex items-center justify-center gap-2 text-white px-6 py-3 sm:py-4 rounded-full transition-all duration-300 font-bold text-sm sm:text-base min-h-[48px] ${project.theme === 'gold'
                  ...
                  <div className="w-full lg:w-1/2 relative z-10 h-56 xs:h-64 sm:h-80 md:h-[400px] lg:h-[500px] mt-8 lg:mt-0">
                    <div className={`w-full h-full rounded-xl sm:rounded-3xl overflow-hidden glass border shadow-2xl relative group-hover:scale-[1.02] transition-transform duration-700 ${project.theme === 'gold' ? 'border-[#D4AF37]/20' : 'border-purple-500/20'
                      }`}>
